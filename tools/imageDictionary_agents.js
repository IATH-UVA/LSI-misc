...
const location = '../dataChp/00_Agents-tab.csv'; /// use tab version for clean read
...
fs.writeFileSync('../dataChp/00_AgentsUTF8.js', 'var agentsUTF8='+stringArray+';\r module.exports=agentsUTF8');
...
fs.writeFileSync('../dataChp/00_AgentsUTF8Min.js', 'var agentsUTF8Min='+stringArr+';\r module.exports=agentsUTF8Min');
const string = JSON.stringify(duplicates);
fs.writeFileSync('../dataChp/00_AgentsUTF8Dup.js', 'var agentsUTF8Dup='+string+';\r module.exports=agentsUTF8Dup');


var old = 0;

var overlap = simple.map(item=>{
	namesChecked.forEach(entry=>{
		if (entry.results.length>0 && (item.name === entry.imgName || item.name === entry.results[0].term)){
			item.old = true;
			console.log(item);
			old ++;
		}
...
console.log(simple.length, old, newItems);

const stringArrMin = JSON.stringify(newItems);
fs.writeFileSync('../dataChp/00_AgentsUTF8Min_.js', 'var agentsUTF8Min_='+stringArrMin+';\r module.exports=agentsUTF8Min_');
